/**
 * Echo vertical-scroll feed layout — pure helpers (AE456).
 *
 * Moved from `apps/web/src/components/aether/phase3/echo-layout.ts` into
 * the Phase 4 shared sub-package so both web R3F and native R3F arrange
 * the TikTok-style card stack with identical world coordinates + scales.
 *
...
export function visibleEchoSlots<T extends { readonly id: string }>(
