/**
 * Atlas weather simulation — pure helpers.
 *
 * AE388 doesn't pull real weather data (that needs an API + needs to
 * land later). Instead it simulates a plausible weather state from the
 * trip's destination slug + the trip's start month. The simulation
 * matches India's actual seasonality so demos feel grounded:
 *
 *   - Kerala / Alleppey: monsoon in Jun-Sep, otherwise clear
 *   - Goa / Anjuna: monsoon in Jun-Sep, otherwise clear
 *   - Mumbai-coast (proxy via Goa data): same
 *   - Leh / Ladakh: clear most months, storm at high altitude in Jul-Aug
 *   - Rajasthan / Jaipur: clear year-round (desert)
 *   - Default: 'clear'
 *
 * The pure helpers in this file map (slug, date) → 'clear' | 'rain' |
 * 'storm'. The Atlas scene mounts WeatherStreaks when the state is
 * rain/storm and skips the field when clear.
 */
