    ...
    <main className="space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted">
          7-day daily or 24-hour hourly forecast for any coordinate. Pulls from the same provider
      ...
      <Card>
        <CardHeader>
          <CardTitle>{mode === 'daily' ? '7-day forecast' : '24-hour forecast'}</CardTitle>
          <CardSubtitle>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <div role="tablist" aria-label="Forecast mode" className="flex gap-1">
            <button
              type="button"
              role="tab"
          ...
          <Button type="button" size="sm" onClick={fetchWeather} disabled={pending}>
  ...
  return (
    <ul className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
      {data.days.map((d) => {
        ...
        return (
          <li key={d.date} className="rounded-md border border-muted/15 bg-surface p-3">
    ...
    <ul className="space-y-1">
      {data.hours.map((h) => {
        ...
        return (
          <li
            key={h.time}
            className="grid grid-cols-[auto_auto_1fr_auto_auto] items-center gap-3 rounded-md border border-muted/10 bg-surface px-3 py-1.5 text-xs"
          >
            ...
            <span className="font-medium">{Math.round(h.tempC)}°C</span>
