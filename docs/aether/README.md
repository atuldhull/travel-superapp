...
40. [`38-phase4-closeout.md`](38-phase4-closeout.md) — **Phase 4 closeout** (AE582): the capstone. Marks Phase 4 **code-complete pending the operator EAS device build + backend b-slices**. Full inventory of all 10 surfaces (renderer / scene / route / interaction / canvas-shared math), the 3 shared hooks, the 4 fixtures, the 4 native packages, the BD audit findings, the documented deferrals, and a 4-part **operator handoff checklist** (ship the binary · validate on device · backend b-slices · tooling debt). Nothing has rendered on a device yet — that is the largest remaining unknown and is operator-gated.
41. [`39-phase5-plan.md`](39-phase5-plan.md) — **Phase 5 begins** (AE583-AE590): Predictive + Perception. Key finding — the architecture already has the sockets (the surface manager's `anticipate(id)`, the `usePerception()` stub, the `predictor-auto` premium capability). This round ships the one authorable keystone: the **Predictor foundation** in `@app/aether-canvas-shared` — a pure heuristic that ranks the user's next surface (transition priors + time-of-day + trip-phase + recency penalty) behind a `SurfacePredictor` interface the future MLP swaps into, plus the feature-vector contract + the confidence gate for `anticipate()`. canvas-shared jest 745 → 801 (+56). A 27-agent adversarial review confirmed 17 findings; the 6 real bugs were fixed (AE590, incl. an unreachable `mirror` candidate). The predictive/perception tiers proper are gated on production telemetry + real devices.
42. [`39-phase5-plan.md`](39-phase5-plan.md) §slice 2 — **Phase 5 perception contract + mock** (AE591-AE596): the framework-free mirror of aether-core's perception types (`GazePoint`/`Gesture`/`GestureEvent`/`PerceptionState`) + the helpers a surface reacts with — `gaze-zone` (gaze → 3×3 zone), `gesture-intent` (gesture → intent), `gesture-recognizer` (anti-jitter fire-once FSM), and `mock-perception` (a deterministic `mockPerceptionStateAt(t)` so consumers build against perception before mediapipe). canvas-shared jest 801 → 844. A 20-agent adversarial review confirmed 11 findings; fixed 2 real bugs (mid-hold fire-once + super-unit confidence) and added a **type-only lock-step test** that fails the build if the mirror drifts from aether-core. 2 of 4 authorable Phase-5 slices done.
43. [`39-phase5-plan.md`](39-phase5-plan.md) §slice 3 — **Phase 5 predictor→surface-manager wiring** (AE597-AE598): `useSurfaceAnticipation` records a recency history, builds the feature vector from the surface-manager state + the clock, runs the predictor + confidence gate, and feeds the actionable pick into the manager's `anticipate()` pre-warm socket; flag-gated by `NEXT_PUBLIC_FEATURE_AETHER_PHASE5`. Lives in **apps/web** (not aether-core — a reverse dep on canvas-shared would be a turbo cycle; apps/web consumes both acyclically + reads the flag). 15 vitest specs incl. a `SurfaceManagerProvider` integration. A 14-agent adversarial review found 6 test-coverage gaps and **0 code bugs** (the wiring was mutation-test-verified); added 3 tests closing the clock/tripPhase/history-cap gaps. 3 of 4 authorable Phase-5 slices done — only the Continuum sync protocol contract remains.
