...
| **AE385** | **Drift Now Card foreground**. Pure `now-card-content.ts`: `timeBandFor(hour)` collapses 0..23 into morning (5..11) / afternoon (12..16) / evening (17..20) / night (21..4 + NaN/Infinity defaults to night); `nowCardContent(date                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | epoch)`returns`{band, headline, suggestion, verb}`per band (morning = Plan + "Sketch the day ahead."; afternoon = Refine + "Refine your plan."; evening = Reflect + "Reflect on today."; night = Dream + "Dream of where next.");`nowCardContentNow()`convenience. New`<DriftNowCard at? hidden?/>` 2D HTML overlay positioned at translate(-50%, -160%) of viewport center; eyebrow band + Playfair-italic suggestion + accent-pill verb button; all colours via AE381 CSS vars (`--aether-palette-surface/ink/accent/glow`) so the AE384 destination tint applies automatically; `aria-label="Now: <headline>"`; `data-aether-now-verb`on the button. Mounted in Phase1DriftShell. **18 new vitest specs**: 6 timeBandFor (band ranges + 4.99/5.0/11.99/12.0 boundary snaps + NaN/Infinity defaults), 7 nowCardContent (each band + epoch input + all-fields-present), 1 nowCardContentNow shape, 5 component (renders all 3 elements; updates on at-prop change; aria-label; data-aether-now-verb attr; hidden returns null). web vitest 1439 → 1457 (+18). Web typecheck clean. 0 new lint errors. Commit`fdd0019`.                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **AE386** | **DriftNowCard breathes with the lifecycle**. Pure `now-card-lifecycle.ts`: `nowCardOpacityForPhase` (idle/dissolving=0, active=1); `nowCardScaleForPhase` (idle/dissolving=0.96, active=1.0); `nowCardTransitionMs` (idle=0 snap; materialisingMs/settlingMs/dissolvingMs otherwise; settlingMs reused for listening); `nowCardCssForPhase` composes `{opacity, transform: 'translate(-50%, -160%) scale(...)', transition}` w/ ease-out cubic for materialising/settling/listening and ease-in cubic for dissolving. `DEFAULT_NOW_CARD_DURATIONS` matches AE375 (700+500+500). DriftNowCard refactored into wrapper + inner: outer decides whether to call `useSurfaceLifecycle()` based on `disableLifecycle` prop, inner takes resolved phase via prop (rules-of-hooks-safe). Existing AE385 specs updated to pass `disableLifecycle`. **16 new vitest specs**: 4 opacity, 2 scale, 5 transition (incl. custom durations), 6 css composition (idle scale-0.96 + 0ms; listening scale-1.0; materialising ease-out cubic 700ms; dissolving ease-in cubic 500ms; translate(-50%, -160%) anchor preserved; transition includes opacity + transform). web vitest 1457 → 1473 (+16). Web typecheck clean. 0 new lint errors. Commit `a2217fe`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **AE387** | **useLifecycleEvents hook** — clean per-phase callback subscription. New `use-lifecycle-events.ts`: `LifecycleEventHandlers` (`onMaterialise/onSettle/onListen/onDissolve/onIdle` + catch-all `onAny(next, prev)`); pure `handlerKeyForPhase(phase)` dispatch table; `useLifecycleEvents(handlers)` reads `useSurfaceLifecycle`, ref-stores handlers so inline callbacks stay fresh without re-effect, fires `onAny` then per-phase handler on every transition (including the first mount so a surface mounting already in `materialising` still gets the callback). **7 new vitest specs**: 1 pure dispatch + 6 integration (onIdle on first mount; onMaterialise after FSM advance; full 5-phase walk under BREATHING plan; onAny shape; inline-handler closure freshness via ref; empty handlers no-op). web vitest 1473 → 1480 (+7). Web typecheck clean. 0 new lint errors. Commit `753ed8f`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **AE388** | **Atlas weather streaks** — per 02-surfaces.md §3 ("weather rendered as ambient particles"). New `@app/aether-canvas/weather-streaks.tsx`: `<WeatherStreaks/>` R3F `<points>` field w/ vertical fall + height-wrap; pure `streakYAt(i, t, speed, height)` per-particle y; storm intensity darkens + thickens; `noise1` matches AmbientField seed for visual coherence. New `weather-simulation.ts`: per-destination monsoon/storm month sets (Kerala/Goa/etc. rain Jun-Sep; Leh storm Jul-Aug; Darjeeling extended Jun-Oct; Rajasthan clear); `weatherForSlugMonth` + `simulatedWeatherFor(slug, Date                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | string                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | null)`; intensity/count derivation (clear=0, rain=400@1.0, storm=700@1.6). New `weather-context.tsx`: `<WeatherProvider weather>`+`useWeather()` default 'clear' (no-throw). Wired in Atlas shell (`simulatedWeatherFor(extractDestinationSlugFromTitle(trip.title), trip.startsOn)` → provider) + Atlas scene (`useWeather`+ conditional`<WeatherStreaks/>`when weatherHasParticles). Dev pip shows the state. **27 new specs**: 7 canvas jest (streakYAt: height-0 guard, [-h/2,h/2] bound across 50×5 samples, decreasing-y majority, wrap-period invariant, phase spread; noise1). 16 vitest weather-simulation (each destination month grid, case-insensitive, fallback, Date+ISO+invalid input). 4 vitest weather-context (default+rain+storm+no-provider). aether-canvas 62 → 69 (+7). web vitest 1480 → 1500 (+20). Typecheck clean (caught + fixed`new Set()`→ typed`EMPTY*MONTHS`). 0 new lint errors. Commit \_next*. | _next_    |
